...
def visualize_region(env, args, get_feasibility, bound=2.5):
    ...
    feasibility = get_feasibility(obs)
    x = np.linspace(-bound, bound, 100)
    y = np.linspace(-bound, bound, 100)
    X, Y = np.meshgrid(x, y)
    ...
    # ax.contour(X, Y, feasibility, level=0, color='green', linewidth=3)
    # plt.colorbar(ctf)
    plt.axis('equal')
    rect1 = plt.Rectangle((0, 0), 1, 1, fc=ctf.collections[0].get_facecolor()[0], ec=ctf.collections[0].get_facecolor()[0], linewidth=3)
    h = [rect1]
    l = ['Feasible Region']
    for key, val in config_dict.items():
        if key.startswith('hazard'):
            c1 = plt.Circle((val[0], val[1]), radius=0.30, fill=False, ec='red', linewidth=1.5)
            ax.add_patch(c1)
            if key[6] == '0':
                h.append(c1)
                l.append('Hazards')
            ...
            c3 = plt.Circle((val[0], val[1]), radius=0.30, fc='yellow', ec='yellow', alpha=0.4, linewidth=1.5)
            ax.add_patch(c3)
            h.append(c3)
            l.append('Goal')
    ...
    plt.savefig(osp.join(args.fpath, 'region.png'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fpath', type=str, default='/home/mahaitong/PycharmProjects/safety-starter-agents/envs/2021-11-20_ppo_dual_ascent_Safexp-CustomPush2-v0/2021-11-20_23-09-05-ppo_dual_ascent_Safexp-CustomPush2-v0_s0')
